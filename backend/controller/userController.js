...
export const GetUserData = catchAsyncErrors(async (req, res, next) => {
  ...
  successResponse(res, 200, "User data fetched succesfully", user)
})
...
export const LoginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) return next(new ErrorHandler("Invalide credetials", 500));

  const isPasswordMatched = await bcrypt.compare(password, user.password);

  if (!isPasswordMatched)
    return next(new ErrorHandler("Invalide credetials", 500));
...
export const LogoutUser = catchAsyncErrors(async (req, res) => {
  ...
  successResponse(res, 200, "Loged out successfully")
})